<template>
    <div>
      ...
      <div class="toou" >
        ...
        <span style="margin-left: 350px;font-size: 14px">离职单编号:</span>
        ...
        <el-table :data="tableData" style="width: 100%; text-align:center;">
          <el-table-column label="编号"  >
            <template slot-scope="scope">
              ...
              <span style="margin-left: 10px">{{ scope.row.dimid }}</span>
            </template>
          </el-table-column>
          <el-table-column label="员工姓名" prop="myemp.ename" > </el-table-column>
          <el-table-column label="离职日期" prop="dimissiontime" width="200px"> </el-table-column>
          ...
          <el-table-column label="离职状态" >
            <template slot-scope="scope">
          <span v-if="scope.row.dshenpi==0">
            <el-tag type="warning">待审批</el-tag>
          </span>
              <span v-else-if="scope.row.dshenpi==1">
            <el-tag type="success">已通过</el-tag>
          </span>
              <span v-else-if="scope.row.dshenpi==2">
      ...
      <el-drawer
        ...
        size="50%">
        <h3  style="margin-left: 20px">新增申请离职</h3>
        <div >
            ...
            <div>
              <div style="width:530px;">
                <div style="margin-top: 20px;margin-left: 30px;font-size: 15px;border-bottom: 1px solid grey">
                  <div>编号: {{tableDataXiang.dimid}}</div>
                </div>
                <div style="margin-top: 20px;margin-left: 30px;font-size: 15px;border-bottom: 1px solid grey">
                ...
                <div style="margin-top: 20px;margin-left: 30px;font-size: 15px;border-bottom: 1px solid grey">
                  <div>离职部门:{{tableDataXiangdept.dname}} </div>
                </div>
                <div style="margin-top: 20px;margin-left: 30px;font-size: 15px;border-bottom: 1px solid grey">
                  <div>离职岗位:{{tableDataXiangmyposition.poname}} </div>
                </div>
                <div style="margin-top: 20px;margin-left: 30px;font-size: 15px;border-bottom: 1px solid grey">
                  <div>员工总结:{{tableDataXiang.dzong}} </div>
                </div>
              </div>
            </div>
      ...
      data(){
        ...
        loadData() {
          ...
          let param = {
            ...
            dimid:this.formInline.tids
          };
          console.log("调岗分页查询：", param)
          let ppp = this.$Qs.stringify(param);
          this.$axios.post("http://localhost:8089/dimission/listselectdimission",ppp)
            .then(r => {
              if (r.status === 200) {
        ...
        Liketijiao(){
          ...
          let param = {
            eid:this.value,
            dimissiontime:this.zhuan.rutime,
            dzong:this.zhuan.jzong
          };
          console.log("点击了立刻提交：", param)
          let ppp = this.$Qs.stringify(param);
          this.$axios.post("http://localhost:8089/dimission/Insertdimission",ppp)
            .then(r => {
              if (r.status === 200) {
                console.log("全部",r.data)
                this.drawer1 = false;
